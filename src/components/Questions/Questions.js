    ...
    const questions = [
        {
            ...
            answer: `React is a javascript library that allow to create UI. Behind the seen it create virtual dom that it shadow of real dom. and render to real dom. after changes some data it not replace whole DOM, it compare virtual dom and real dom using diff algorithm that is highly optimized and after comparing it only update on real dom that is changed.`,
        },
        {
            id: 2,
            question: `What is different between state and props ?`,
            answer: `Props are use to pass data parent component to child component ? it readonly and immutable. state are local data store that is used own only the component.
            ...
            <div className="accordion">
                {questions.map((item) => (
                    <div className="accordion-item">
                        <li
                            className={`accordion-header ${openIds.includes(item.id) ? "active-item" : ""}`}
